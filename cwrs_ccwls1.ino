...
void setup() {
  ...
  digitalWrite(RELAY_PIN[0], LOW); // LOW initially
  Serial.begin(115200);
  Serial.flush();
...
void motorStep(bool isClockwise, int steps) {
  ...
  if (isClockwise)
    _step_current_position += steps; // Update the actual motor position for clockwise movement
  else
    _step_current_position -= steps; // Update the actual motor position for counterclockwise movement
}
...
void ionizerLamp(bool state) {
  digitalWrite(RELAY_PIN[0], state ? HIGH : LOW);
}
